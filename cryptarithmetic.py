...
def fill_in(formula):
    ...
    letters = ''.join(set(letter for letter in formula if letter in string.uppercase))
...
def compile_formula(formula, verbose=False):
    ...
    if verbose: print f
    return eval(f), letters # finally, use 'eval' to make this function
...
def compile_word(word):
    ...
    if word.isupper():
        ...
        for i in range(len(word)):
            terms.append(str(10**(len(word)-1-i)) + '*' + word[i] )
